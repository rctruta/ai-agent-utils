...
The `-s workflow` matters: GitHub **refuses** any push that creates or updates
`.github/workflows/` files from a token without the `workflow` scope — and
Python projects generated by this script include a CI workflow. Without the
scope, `gh repo create --push` creates the remote and then dies half-way
(verified live: `refusing to allow an OAuth App to create or update workflow`).
Already logged in with an older token? Add the scope in place:

```bash
gh auth refresh -h github.com -s workflow
```

The script checks for this scope before creating a remote for a Python
project and refuses early with these exact instructions, rather than leaving
you with a half-pushed repo.
